...
Usage:
    python sector_rotation_v0.5.py
...
Author: Brian + Claude
Date: 2026-01-20
Version: 0.5 (Masked similarity for robust pattern matching)
"""
...
def calculate_predictions(similar_days, forward_df, ticker, history_cutoff=None):
...
def build_sector_rotation_dashboard(all_data):
    ...
    for i, ticker in enumerate(all_data.keys()):
        ...
        for period_name, cutoff in cutoffs.items():
            preds = calculate_predictions(similar_days, forward_df, ticker, cutoff)
...
if __name__ == '__main__':
    print('=' * 80)
    print('Sector Rotation Backend v0.5y')
    print('=' * 80)
    print()
