...
v1.7: Added pullback buy alert (PB column, third signal dot)
      - pullback_flag: 'buy_now' | 'none'
      - Conditions: score>=60, sma_all, momentum=clean, ret_3m>0,
        ret_1w<0, rsi_5<40, abs(pct_from_sma21)<=3%, vol!=thin
      - New fields: rsi_5, pct_from_sma21
      - Score gate applied post add_scores()
      - Pulsing green dot for buy_now; stat bar count
v1.6: Split Sig column into two sortable columns: M (momentum) and V (volume)
...
# Pullback alert thresholds (v1.7)
PULLBACK_RSI_PERIOD        = 5     # short-term RSI period
PULLBACK_RSI_THRESHOLD     = 40.0  # RSI must be below this
PULLBACK_SMA_WINDOW        = 21    # SMA to measure pullback proximity toward
PULLBACK_SMA_PCT_THRESHOLD = 0.03  # within 3% of SMA21 (above or below)
PULLBACK_MIN_SCORE         = 60.0  # minimum score gate (applied post add_scores)
PULLBACK_MIN_RET_3M        = 0.0   # 3m return must be positive (percent)
...
def compute_rsi(series: pd.Series, period: int = 5) -> float:
    """
    Compute RSI over the last `period` bars.
    Uses simple average of gains/losses (Wilder's method approximation).
    Returns float 0-100 or NaN.
    """
    try:
        if len(series) < period + 2:
            return np.nan
        delta = series.iloc[-(period + 1):].diff().dropna()
        if len(delta) < period:
            return np.nan
...
def compute_pullback_signal(df: pd.DataFrame, row: dict) -> tuple:
    """
    Evaluate pullback buy conditions for a single ticker.
    Score gate is NOT applied here — applied post add_scores() in a second pass.

    Returns:
        (pullback_flag: str, rsi_5: float, pct_from_sma21: float)
        pullback_flag is 'buy_now' or 'none' (pre-score-gate)
    """
    try:
        # --- Compute 5-day RSI ---
        rsi = compute_rsi(df['close'], PULLBACK_RSI_PERIOD)

        # --- Distance from 21-day SMA ---
        if len(df) >= PULLBACK_SMA_WINDOW:
            sma21 = float(df['close'].iloc[-PULLBACK_SMA_WINDOW:].mean())
            price = float(df['close'].iloc[-1])
            pct_from_sma21 = (price - sma21) / sma21 if sma21 != 0 else np.nan
        else:
            pct_from_sma21 = np.nan

        # --- Gate conditions (structural, must all pass) ---
        gate_sma_all    = row.get('sma_all', False)
        gate_momentum   = row.get('momentum_flag', '') == 'clean'
        gate_ret_3m     = (row.get('ret_3m') or 0.0) > PULLBACK_MIN_RET_3M

        # --- Trigger conditions (short-term, must all fire) ---
        trigger_1w_neg  = (row.get('ret_1w') or 0.0) < 0
        trigger_rsi     = (not np.isnan(rsi)) and rsi < PULLBACK_RSI_THRESHOLD
        trigger_sma_prox = (not np.isnan(pct_from_sma21)) and abs(pct_from_sma21) <= PULLBACK_SMA_PCT_THRESHOLD
        trigger_vol_ok  = row.get('vol_flag', '') != 'thin'

        gates_pass    = gate_sma_all and gate_momentum and gate_ret_3m
        triggers_pass = trigger_1w_neg and trigger_rsi and trigger_sma_prox and trigger_vol_ok

        flag = 'buy_now' if (gates_pass and triggers_pass) else 'none'

        rsi_out = round(rsi, 2) if not np.isnan(rsi) else None
        pct_out = round(pct_from_sma21 * 100, 2) if not np.isnan(pct_from_sma21) else None

        return flag, rsi_out, pct_out
...
def compute_all_metrics(price_cache: dict, benchmark_cache: dict) -> pd.DataFrame:
    ...
    for ticker, df in tqdm(price_cache.items(), desc='Computing metrics'):
        ...
        # Pullback alert signal (score gate applied later in add_scores)
        pb_flag, rsi_5, pct_from_sma21 = compute_pullback_signal(df, row)
        row['pullback_flag']   = pb_flag
        row['rsi_5']           = rsi_5
        row['pct_from_sma21']  = pct_from_sma21
...
def add_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Pullback score gate — downgrade buy_now to none if score too low
...
RENDER_JS_TAIL = """;
    ...
    function buildTooltipHTML(row) {
        ...
        const rsi5    = row.rsi_5;
        const sma21pct = row.pct_from_sma21;
        const pbLabel = pbflag === 'buy_now' ? 'BUY ON PULLBACK' : 'No signal';
        const pbClass = pbflag === 'buy_now' ? 'tt-signal-clean' : 'tt-vol-normal';
        const rsiDisplay  = rsi5    != null ? rsi5.toFixed(1)              : 'n/a';
        const smaDisplay  = sma21pct != null ? (sma21pct >= 0 ? '+' : '') + sma21pct.toFixed(2) + '% vs SMA21' : 'n/a';
        html += `<div class="tt-section">Pullback Alert</div>`;
        html += `<div class="tt-row"><span class="${pbClass}">${pbLabel}</span></div>`;
        html += `<div class="tt-row"><span style="color:#94a3b8;">RSI(5)</span><span>${rsiDisplay}</span></div>`;
        html += `<div class="tt-row"><span style="color:#94a3b8;">vs SMA21</span><span>${smaDisplay}</span></div>`;
    ...
    function renderTable() {
        ...
        const TIP = {
            ...
            pb:    'Pullback Buy Alert. Pulsing green dot = buy_now. '
                 + 'Fires when ALL conditions met: score>=60, all SMAs intact, '
                 + 'momentum=clean, 3m return positive, 1w return negative (pulled back), '
                 + '5-day RSI<40 (washed out), price within 3% of 21-day SMA, '
                 + 'volume not thin. Sortable by RSI. Hover ticker for RSI and SMA details.',
        };
...
<tr class="grp-row">
  ...
  ${th('rsi_5',         'PB',    '',        TIP.pb)}
        ...
        visible.forEach((row, i) => {
            ...
            const pbflagTd = `<td title="Pullback: ${pbflag}"><span class="pbflag-dot pbflag-${pbflag}"></span></td>`;
...
def build_body_html(output, writer):
    ...
    pullback_buy_ct  = sum(1 for r in data if r.get('pullback_flag') == 'buy_now')
    ...
    parts.append(writer.stat_bar([
        ...
        ('Pullback Buys',  str(pullback_buy_ct),   'pos'  if pullback_buy_ct > 0  else 'neutral'),
    ]))
    ...
    table_html = (
        ...
        ' &nbsp;|&nbsp; PB = pullback buy alert (sortable by RSI) \u2014 '
        ' &nbsp;<span style="color:#16a34a;font-weight:700;" title="Pullback Buy: all conditions met \u2014 score>=60, SMAs intact, momentum clean, 3m positive, 1w negative, RSI(5)<40, within 3% of SMA21, volume not thin.">&#9679;</span> buy now'
        ' &nbsp;<span style="color:#e2e8f0;font-weight:700;" title="Pullback Buy: no signal \u2014 conditions not met.">&#9675;</span> none'
        '</span></div>'
        + filter_html
...
def main():
    ...
    preview = ['rank','ticker','score','price','ret_1w','ret_1m','ret_3m','ret_1y','sma_all','ratio_passes','momentum_flag','vol_flag','vol_ratio','pullback_flag','rsi_5','pct_from_sma21']
