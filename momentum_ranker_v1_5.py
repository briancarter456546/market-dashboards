...
.mr-table colgroup col.col-mflag  { width: 36px; }
.mr-table thead th {
    padding: 7px 5px;
    font-size: 0.72em;
    font-weight: 700;
...
RENDER_JS_TAIL = """;
    ...
    function renderTable() {
        ...
        const TIP = {
            ...
            sig:   'Two signal dots. '
                 + 'LEFT dot = Momentum character (3-day return z-score vs 63-day distribution): '
                 + 'green=clean, amber=stalling (z < -1.5), red=overextended (z > +2.0). '
                 + 'RIGHT dot = Volume confirmation (5-day avg vs 63-day avg): '
                 + 'blue=confirmed (>1.5x), gray=normal, white ring=thin (<0.75x). '
                 + 'Hover the ticker for full details.',
        };
...
<tr class="grp-row">
  ...
  <th class="grp-signal" title="${TIP.sig}">Sig</th>
</tr><tr>
  ...
  ${th('momentum_signal','M',    '',        TIP.sig)}
        ...
        visible.forEach((row, i) => {
            ...
            // Momentum + volume flag dots (two dots in one cell)
            const flag  = row.momentum_flag || 'clean';
            const vflag = row.vol_flag || 'normal';
            const mflagTd = `<td title="momentum: ${flag} | volume: ${vflag}" style="white-space:nowrap;"><span class="mflag-dot mflag-${flag}"></span><span class="vflag-dot vflag-${vflag}"></span></td>`;
...
def build_body_html(output, writer):
    ...
    table_html = (
        ...
        ' &nbsp;|&nbsp; Sig (M) = momentum + volume \u2014 hover ticker for details'
        ...
        ' &nbsp;|&nbsp; <span style="color:#2563eb;font-weight:700;" title="Volume: Confirmed \u2014 5-day avg volume is 1.5x or more above the 63-day avg. Strong conviction behind the move.">&#9679;</span> vol confirmed'
        ' &nbsp;<span style="color:#94a3b8;" title="Volume: Normal \u2014 5-day avg volume is within 0.75x\u20131.5x of the 63-day avg. Nothing unusual.">&#9679;</span> vol normal'
        ' &nbsp;<span style="color:#cbd5e1;" title="Volume: Thin \u2014 5-day avg volume is below 0.75x the 63-day avg. Price moving on light volume. Low conviction.">&#9675;</span> vol thin'
        '</span></div>'
        + filter_html
...
def main():
    ...
    preview = ['rank','ticker','score','price','ret_1w','ret_1m','ret_3m','ret_1y','sma_all','ratio_passes','momentum_flag','vol_flag','vol_ratio']
