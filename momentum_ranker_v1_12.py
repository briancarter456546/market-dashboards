...
<tr class="grp-row">
  ...
  ${th('ol_score',      'OL',    '',        TIP.ol)}
        ...
        visible.forEach((row, i) => {
            ...
            const mColor   = flag === 'overextended' ? '#dc2626' : flag === 'stalling' ? '#d97706' : '#16a34a';
            const mVal     = (row.momentum_signal != null) ? (row.momentum_signal >= 0 ? '+' : '') + row.momentum_signal.toFixed(2) : '—';
            const mflagTd  = `<td title="Momentum z-score: ${mVal} (${flag})" style="color:${mColor};font-weight:700;font-size:0.78em;text-align:center;">${mVal}</td>`;
  ...
  ${olflagTd}
    ...
    // -------------------------------------------------------------------------
    // THEME OVERLAP (cosine similarity on 20-dim corr_vec)
    // -------------------------------------------------------------------------
