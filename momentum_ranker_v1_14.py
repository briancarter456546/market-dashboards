  ...
  ${olflagTd}
    ...
    function updatePoleCorrRow() {
        ...
        _corrUniverse.forEach(function(pole, i) {
            const cell = document.getElementById('pole-corr-' + pole);
            if (!cell) return;
            if (owned.length === 0) {
                cell.textContent = '--';
            ...
            const avg = sum / cnt;
            const disp = avg.toFixed(2);
            // Color: high corr = more saturated
            const intensity = Math.min(1, Math.max(0, avg));
            const r = Math.round(22  + intensity * (37  - 22));
            const g = Math.round(163 * intensity);
            const b = Math.round(74  * intensity);
            cell.textContent = disp;
            cell.style.color = '#fff';
            cell.style.background = avg >= 0.5
                ? `rgb(${Math.round(22 + intensity*14)},${Math.round(163*intensity)},${Math.round(74*intensity)})`
                : '#1e293b';
        });
    }
