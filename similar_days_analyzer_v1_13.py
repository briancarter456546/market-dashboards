...
Usage:
    python similar_days_analyzer_v1_13.py
...
Changes in v1.13:
    - Added Perplexity Sonar API integration for auto-generated LLM analysis
    - Same-day cache to avoid burning API quota on re-runs
    - Existing copy-paste prompt section preserved unchanged
...
Author: Brian + Claude
Date: 2026-03-03
Version: 1.13 (Perplexity Sonar API integration)
"""
...
_PERPLEXITY_CACHE_PATH = os.path.join(_SCRIPT_DIR, 'similar_days_perplexity_cache.json')


def _call_sonar(prompt_text):
    """Call Perplexity Sonar API with the LLM research prompt.
    ...
    if not api_key:
        print('[INFO] PERPLEXITY_API_KEY not set -- skipping Sonar analysis')
        return None
    ...
    payload = {
        'model': 'sonar',
        'messages': [
            {'role': 'system', 'content': 'You are a financial market historian and analyst. Provide detailed, well-structured analysis with specific dates, events, and data points.'},
    ...
    try:
        ...
        print(f'[OK] Sonar API returned {len(content)} chars, {len(citations)} citations')
        return {'content': content, 'citations': citations}
    except Exception as e:
        print(f'[WARN] Sonar API call failed: {e}')
        return None


def _get_sonar_analysis(prompt_text):
    """Get Sonar analysis with same-day caching.

    Checks cache file -- if today's date matches, reuse cached response.
    Otherwise calls API and writes cache.
    """
    ...
    if os.path.exists(_PERPLEXITY_CACHE_PATH):
        try:
            ...
            if cached.get('date') == today_str and cached.get('content'):
                print(f'[CACHE] Reusing Sonar analysis from {today_str}')
                return {'content': cached['content'], 'citations': cached.get('citations', [])}
        except Exception:
            pass  # Corrupted cache -- just re-fetch

    # Call API
    result = _call_sonar(prompt_text)
    if result is None:
        return None
    ...
    try:
        ...
        print(f'[OK] Cached Sonar response to {_PERPLEXITY_CACHE_PATH}')
    except Exception as e:
        print(f'[WARN] Failed to write Sonar cache: {e}')
...
def build_body_html(output, latest_date, writer):
    ...
    if sonar_result and sonar_result.get('content'):
        ...
        if citations:
            citation_items = ''.join(
                f'<li style="margin-bottom:4px;"><a href="{_html.escape(c)}" target="_blank" '
                f'style="color:#4f46e5;text-decoration:none;">{_html.escape(c)}</a></li>'
                for c in citations
            )
            citation_html = (
                '<div style="margin-top:16px;padding-top:12px;border-top:1px solid #e2e4e8;">'
                '<strong style="font-size:0.85em;">Sources:</strong>'
                f'<ol style="font-size:0.8em;color:#555;margin-top:6px;padding-left:20px;">{citation_items}</ol>'
                '</div>'
            )

        perplexity_html = (
            '<div class="table-section" style="margin-bottom:22px;">'
            '<div class="table-section-header"><h2>Perplexity Analysis</h2>'
            '<span style="font-size:0.85em;color:#888;">Auto-generated via Perplexity Sonar API</span>'
            '</div>'
            '<div style="padding:16px;">'
