"""
momentum_ranker_long_v1_18.py
=============================
Momentum Ranker (Long Timeframe) - Backend Data Generator

Same as momentum_ranker_v1_18 but with longer return periods:
1m, 3m, 6m, 1y, 5y, 10y (instead of 1d, 1w, 1m, 3m, 6m, 1y).
vs SPY columns: 1y, 5y, 10y.

Usage:
    python momentum_ranker_long_v1_18.py

Output:
    momentum_ranker_long_data.json
    momentum_ranker_long_data.csv
...
============================================================================
v1.18-long: Long timeframe variant. Returns: 1m-10y. vs SPY: 1y, 5y, 10y.
            Scoring weights shorter periods heavier.
            Regime-conditional SMA gate: ON in growth (SPY>200, vol<20%), OFF in stress.
            Scimode validated: below-SMA stocks have 4x better returns in stress regime.
...
# Display periods (shown in table columns and scored)
PERIODS = {
    '1m':  21,
    '3m':  63,
    '6m':  126,
...
# Long-term ratio tests: is momentum building across longer timeframes?
RATIO_THRESHOLDS = {
    '3m_1m':  2.0,   # 3-month return >= 2x 1-month return
    '6m_3m':  1.5,   # 6-month return >= 1.5x 3-month return
    '1y_6m':  1.5,   # 1-year return >= 1.5x 6-month return
    '5y_1y':  2.0,   # 5-year return >= 2x 1-year return
    '10y_5y': 1.5,   # 10-year return >= 1.5x 5-year return
}
...
# 126-day correlation computed against each; top 3 and bottom 3 shown on hover
CORR_UNIVERSE = [
    'QQQ', 'SPY',
...
CORR_LOOKBACK = 126  # trading days for correlation window (6 months)

# Momentum character signal thresholds (21-day return z-score vs 252-day dist)
MOMENTUM_Z_OVERBOUGHT =  2.0   # above this -> overextended (don't chase)
MOMENTUM_Z_STALLING   = -1.5   # below this -> stalling (wait)

# Volume confirmation thresholds
VOL_SHORT = 21   # recent volume window (trading days)
VOL_LONG  = 126  # baseline volume window (trading days)
...
PULLBACK_MIN_RET_3M = 0.0   # 1y return must be positive (variable name kept for compat)
...
def compute_momentum_signal(df: pd.DataFrame) -> tuple:
    """
    Compute 21-day return z-score vs the distribution of all rolling 21-day
    returns over the last 252 trading days.
    ...
    """
    try:
        m_lookback = 252  # 1 year of rolling windows
        m_window   = 21   # 21-day return periods

        if len(df) < m_lookback + m_window + 5:
            return np.nan, 'clean'

        close = df['close'].iloc[-(m_lookback + m_window + 5):]
        daily_ret = close.pct_change().dropna()

        # Build all rolling 21-day returns over the lookback window
        rolling_rets = []
        for i in range(m_window - 1, len(daily_ret)):
            chunk = daily_ret.iloc[i - m_window + 1: i + 1]
        ...
        # Most recent 21-day return
        recent = rolling_rets[-1]
        z = (recent - mu) / sig
        z = round(float(z), 2)
...
# ============================================================================
# POLE MOMENTUM SIGNAL (126-day z-score for CORR_UNIVERSE assets)
# ============================================================================

def compute_pole_momentum(df: pd.DataFrame) -> float:
    """
    Compute 126-day (6-month) return z-score for a pole asset vs distribution
    of all rolling 126-day returns over the last 252 trading days.

    Returns:
        z-score (float) or None if insufficient data
    """
    try:
        p_window   = 126  # 6-month return periods
        p_lookback = 252  # 1 year of rolling windows
        needed = p_lookback + p_window + 5
        if len(df) < needed:
            return None
        ...
        # Build all rolling 126-day returns
        rolling_rets = []
        for i in range(p_window - 1, len(daily_ret)):
            chunk = daily_ret.iloc[i - p_window + 1: i + 1]
            r = float((1 + chunk).prod() - 1)
            rolling_rets.append(r)
...
def compute_pullback_signal(row: dict) -> str:
    """
    Long-timeframe nadir detection for momentum stocks.
    Fires when the stock is down this month BUT the 21-day momentum z-score
    is positive (decline has reversed, now recovering). Plus broader trend intact.
    Vol confirmed is excluded -- surging volume on a down month = distribution.
    Score gate (>=60) is applied as a second pass in add_scores().

    Returns: 'buy_today' or 'none'
    """
    try:
        down_month   = (row.get('ret_1m') or 0.0) < 0
        recovering   = (row.get('momentum_signal') or -999) > 0  # z > 0 = reversing
        sma_ok       = row.get('sma_all', False)
        momentum_ok  = row.get('momentum_flag', '') == 'clean'
        trend_ok     = (row.get('ret_1y') or 0.0) > 0
        not_dist     = row.get('vol_flag', '') != 'confirmed'

        if down_month and recovering and sma_ok and momentum_ok and trend_ok and not_dist:
            return 'buy_today'
        return 'none'
    except Exception:
        return 'none'
...
# ============================================================================
# STRONG BUY SIGNAL (consistent momentum, no pullback needed)
# ============================================================================

STRONG_MIN_SCORE = 80.0  # minimum score gate (applied post add_scores)

def compute_strong_signal(row: dict) -> str:
    """
    Consistent momentum signal for tickers that never meaningfully pull back.
    Fires when ALL return periods are positive, all SMAs intact, momentum clean.
    These are structural winners -- waiting for a pullback may mean never buying.
    Score gate (>=80) is applied as a second pass in add_scores().

    Returns: 'strong_buy' or 'none'
    """
    try:
        sma_ok      = row.get('sma_all', False)
        momentum_ok = row.get('momentum_flag', '') == 'clean'
        not_overbought = (row.get('momentum_signal') or 999) < MOMENTUM_Z_OVERBOUGHT

        # All display periods must be positive
        all_positive = True
        for label in PERIODS:
            ret = row.get(f'ret_{label}')
            if ret is None or ret <= 0:
                all_positive = False
                break

        # Ratio consistency: at least 3 of 5 pass
        ratio_ok = (row.get('ratio_passes') or 0) >= 3

        if all_positive and sma_ok and momentum_ok and not_overbought and ratio_ok:
            return 'strong_buy'
        return 'none'
    except Exception:
        return 'none'
...
def compute_all_metrics(price_cache: dict, benchmark_cache: dict) -> pd.DataFrame:
    ...
    for ticker, df in tqdm(price_cache.items(), desc='Computing metrics'):
        ...
        # Benchmark deltas (long timeframe: 1y, 5y, 10y)
        bm_periods = [('1y', 252), ('5y', 1260), ('10y', 2520)]
        ...
        # Strong buy: consistent momentum across all timeframes, no pullback needed
        row['strong_flag'] = compute_strong_signal(row)
...
def add_scores(df: pd.DataFrame, regime: dict = None) -> pd.DataFrame:
    ...
    # Component 1: weighted avg of return percentiles (shorter periods heavier)
    ret_weights = {'1m': 3.0, '3m': 2.5, '6m': 2.0, '1y': 1.5, '5y': 1.0, '10y': 0.5}
    ...
    weights  = np.array([ret_weights[p] for p in PERIODS.keys()])
    df['score_returns'] = ret_pct.values.dot(weights) / weights.sum()
    ...
    # Component 4: history depth -- tickers with more data score higher
    # Count how many of the 6 return periods have non-null values
    depth_cols = [f'ret_{p}' for p in PERIODS.keys()]
    df['history_depth'] = df[depth_cols].notna().sum(axis=1)
    df['score_depth'] = (df['history_depth'] / len(PERIODS)) * 100  # 0-100

    raw_score = (
        df['score_returns'] * 0.30
        + df['score_ratios'] * 0.20
        + df['score_spy_days'] * 0.20
        + df['score_depth'] * 0.30
    )

    # Regime-conditional SMA gate
    # Growth: gate ON (zero out below-SMA stocks -- momentum continuation works)
    # Stress: gate OFF (let all stocks score -- mean reversion dominates)
    if sma_gate_on:
    ...
    # Strong buy score gate -- require higher score
    df['strong_flag'] = df.apply(
        lambda r: r['strong_flag'] if r['score'] >= STRONG_MIN_SCORE else 'none',
        axis=1
    )
...
RENDER_JS_TAIL = """;
    ...
    const RET_COLS = ['ret_1m','ret_3m','ret_6m','ret_1y','ret_5y','ret_10y'];
    const VS_COLS  = ['vs_spy_1y','vs_spy_5y','vs_spy_10y'];
    ...
    function getSortVal(row, col) {
        ...
        if (col === 'strong_flag') return row.strong_flag === 'strong_buy' ? 1 : 0;
    ...
    function buildTooltipHTML(row) {
        ...
        html += `<div class="tt-section">Volume (21d vs 126d avg)</div>`;
        ...
        if (top.length) {
            html += `<div class="tt-section">Most Correlated (126d)</div>`;
            top.forEach(item => {
        ...
        if (bot.length) {
            html += `<div class="tt-section">Least Correlated (126d)</div>`;
            bot.forEach(item => {
                const cls = item.corr >= 0 ? 'tt-corr-pos' : 'tt-corr-neg';
                const sign = item.corr >= 0 ? '+' : '';
                html += `<div class="tt-row"><span class="tt-asset">${item.asset}</span><span class="${cls}">${sign}${item.corr.toFixed(3)}</span></div>`;
    ...
    function renderTable() {
        ...
        const TIP = {
            ...
            score: 'Composite (0-100) of four components: '
                 + '(1) weighted return percentile across 6 long periods [30%], '
                 + '(2) long-term ratio passes [20%], '
                 + '(3) bad-SPY day resilience [20%], '
                 + '(4) history depth -- tickers with 5y/10y data score higher [30%]. '
            ...
            ratio: 'Long-term momentum consistency: how many of 5 ratio tests pass. '
                 + 'Tests: 3m/1m >= 2x, 6m/3m >= 1.5x, 1y/6m >= 1.5x, 5y/1y >= 2x, 10y/5y >= 1.5x. '
                 + 'Green=4-5 passes, amber=2-3, red=0-1. N/A periods count as fails.',
            badspy:'Average return on the 15 worst SPY days in the last 63 sessions. '
            ...
            sig:   'LEFT dot = Momentum character (21-day return z-score vs 252-day distribution): '
            ...
            vol:   'Volume confirmation dot. '
                 + '21-day avg volume vs 126-day avg volume ratio. '
            ...
            pb:    'Nadir Detection: down month + recovering momentum. '
                 + 'Pulsing green dot = buy_today: stock is down this month (ret_1m<0) '
                 + 'BUT 21-day momentum z-score is positive (decline has reversed). '
                 + 'Plus: all SMAs intact, momentum clean, 1y trend positive. '
                 + 'Volume confirmed excluded (distribution). Score gate: >=60. Sortable by 1m return.',
            sb:    'Strong Buy: consistent momentum across ALL timeframes. '
                 + 'Blue dot = all return periods positive, all SMAs intact, momentum clean, '
                 + 'not overextended, 3+ ratio passes. Score gate: >=80. '
                 + 'These are structural winners where waiting for a pullback may mean never buying.',
  ...
  <col class="col-bsd">
  <col class="col-mzscore"><col class="col-mflag"><col class="col-mflag"><col class="col-mflag"><col class="col-mflag">
</colgroup><thead>
<tr class="grp-row">
  ...
  <th class="grp-signal" title="${TIP.sb}">SB</th>
  ...
  ${th('ret_1m','1m')} ${th('ret_3m','3m')} ${th('ret_6m','6m')}
  ${th('ret_1y','1y')} ${th('ret_5y','5y')} ${th('ret_10y','10y')}
  ${th('ratio_passes',  'Ratio', '',        TIP.ratio)}
  ${th('vs_spy_1y', '1y','',TIP.vsSpy)} ${th('vs_spy_5y','5y','',TIP.vsSpy)} ${th('vs_spy_10y','10y','',TIP.vsSpy)}
  ${th('bad_spy_score', 'Score', '',        TIP.badspy)}
  ${th('momentum_signal','M',    '',        TIP.sig)}
  ${th('vol_ratio',     'V',     '',        TIP.vol)}
  ${th('ret_1m',        'PB',    '',        TIP.pb)}
  ${th('strong_flag',   'SB',    '',        TIP.sb)}
  ${th('ol_score',      'OL',    '',        TIP.ol)}
        ...
        visible.forEach((row, i) => {
            ...
            const sbflag = row.strong_flag || 'none';
            const sbflagTd = sbflag === 'strong_buy'
                ? `<td title="Strong Buy: consistent momentum across all timeframes"><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:#2563eb;"></span></td>`
                : `<td><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:#1e293b;"></span></td>`;
            const olflagTd = `<td title="Theme overlap: ${olflag}"><span class="olflag-dot olflag-${olflag}"></span></td>`;

            // Top pole cell — bg colored by that pole's 6m z-score
            ...
            const tpBg   = (tpZ == null)  ? '#888888'
                         ...
                         :                  '#dc2626';
            const tpTd   = tp
                ? `<td title="Top pole: ${tp} (corr ${tpCorr}, pole 6m z: ${tpZ != null ? tpZ.toFixed(2) : '--'})" style="background:${tpBg};color:#fff;font-weight:700;font-size:0.75em;text-align:center;white-space:nowrap;">${tp} ${tpCorr != null ? tpCorr.toFixed(2) : ''}</td>`
  ...
  ${gradTd(row.ret_5y,'ret_5y')}  ${gradTd(row.ret_10y,'ret_10y')}
  <td style="background:${rpBg};color:#fff;font-weight:700;">${rp}/5</td>
  ${gradTd(row.vs_spy_1y,'vs_spy_1y')}
  ${gradTd(row.vs_spy_5y,'vs_spy_5y')}
  ${gradTd(row.vs_spy_10y,'vs_spy_10y')}
...
def build_body_html(output, writer):
    ...
    strong_buy_ct    = sum(1 for r in data if r.get('strong_flag') == 'strong_buy')
    ...
    parts.append(writer.stat_bar([
        ...
        ('Nadir Buy',      str(pullback_buy_ct),   'pos'  if pullback_buy_ct > 0  else 'neutral'),
        ('Strong Buy',     str(strong_buy_ct),     'pos'  if strong_buy_ct > 0   else 'neutral'),
    ]))
    parts.append(writer.build_header(
        'Ranked by composite momentum score \u2014 {} tickers scanned'.format(n)
    ))

    # ---- POLE PANEL (3-row table: tickers | 6m momentum z | owned corr avg) ----
    ...
    table_html = (
        ...
        ' &nbsp;<span style="color:#16a34a;font-weight:700;" title="Momentum: Clean \u2014 21-day return is within normal range (z-score between -1.5 and +2.0)">&#9679;</span> clean'
        ' &nbsp;<span style="color:#d97706;font-weight:700;" title="Momentum: Stalling \u2014 21-day return z-score below -1.5. Move may be losing steam.">&#9679;</span> stalling'
        ' &nbsp;<span style="color:#dc2626;font-weight:700;" title="Momentum: Overextended \u2014 21-day return z-score above +2.0. Unusual spike relative to this stock\'s history. Risk of chasing.">&#9679;</span> overextended'
        ' &nbsp;|&nbsp; V = volume confirmation (sortable) \u2014 '
        ' &nbsp;<span style="color:#2563eb;font-weight:700;" title="Volume: Confirmed \u2014 21-day avg volume is 1.5x or more above the 126-day avg. Strong conviction behind the move.">&#9679;</span> confirmed'
        ' &nbsp;<span style="color:#94a3b8;" title="Volume: Normal \u2014 21-day avg volume is within 0.75x\u20131.5x of the 126-day avg. Nothing unusual.">&#9679;</span> normal'
        ' &nbsp;<span style="color:#cbd5e1;" title="Volume: Thin \u2014 21-day avg volume is below 0.75x the 126-day avg. Price moving on light volume. Low conviction.">&#9675;</span> thin'
...
def main():
    print('=' * 80)
    print('Momentum Ranker LONG v1.18')
    ...
    # Compute 126-day (6m) momentum z-score for each CORR_UNIVERSE pole
    print('\nComputing pole momentum (126-day z-scores)...')
    pole_momentum = {}
    ...
    writer   = DashboardWriter('momentum-ranker-long', 'Momentum Ranker (Long)')
    ...
    preview = ['rank','ticker','score','price','ret_1m','ret_3m','ret_1y','ret_5y','ret_10y','sma_all','ratio_passes','momentum_flag','pullback_flag','strong_flag']
