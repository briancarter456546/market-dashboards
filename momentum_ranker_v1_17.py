    ...
    for ticker, df in tqdm(price_cache.items(), desc='Computing metrics'):
        ...
        if not corr_ref.empty and len(df) >= CORR_LOOKBACK + 5:
            ...
            if valid_poles:
                ...
                # DEBUG: diagnose why corr_vec is all None
                shared_dates = ticker_ret.index.intersection(corr_ref.index)
                print(f'  [NO CORR] {ticker}: df={len(df)} rows, ticker_ret={len(ticker_ret)} rows, '
                      f'shared_dates={len(shared_dates)}, '
                      f'ticker_idx_sample={list(ticker_ret.index[:2])}, '
                      f'corr_ref_idx_sample={list(corr_ref.index[:2])}')
        else:
            row['corr_top']      = []
            row['corr_bot']      = []
