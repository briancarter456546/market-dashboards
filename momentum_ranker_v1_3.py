...
Usage:
    python momentum_ranker_v1_3.py
...
<tr class="grp-row">
  ...
  ${th('momentum_signal','M')}
        ...
        visible.forEach((row, i) => {
            ...
            // Momentum flag dot
            const flag = row.momentum_flag || 'clean';
            const mflagTd = `<td title="${flag}"><span class="mflag-dot mflag-${flag}"></span></td>`;
...
def build_body_html(output, writer):
    ...
    table_html = (
        ...
        ' &nbsp;|&nbsp; Sig (M) = momentum character signal \u2014 hover ticker for details'
...
def main():
    ...
    preview = ['rank','ticker','score','price','ret_1w','ret_1m','ret_3m','ret_1y','sma_all','ratio_passes','momentum_flag']
